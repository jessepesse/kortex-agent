...
from kortex.logging import get_logger, setup_logging
    ...
    for provider, models in config["models"].items():
        ...
        for model in models:
            ...
            if isinstance(model, dict):
                ...
                console_write(f"    {idx}. id: {m_safe}{thinking_str}")
                all_models.append((provider, model_id))
            else:
                m_safe = safe_str(model)
                console_write(f"    {idx}. {m_safe}")
...
def main():
    ...
    setup_logging(os.getenv("KORTEX_LOG_LEVEL", "INFO"))
    ...
    try:
        ...
        logger.exception("Failed to initialize selected AI provider/model")
    ...
    p_safe = safe_str(provider).upper()
    m_safe = safe_str(model)
    console_write(f"🧠 LIFE OS - {p_safe} {m_safe}")
    console_write("="*60)
    console_write("Commands: quit, exit, reload, model (change model)")
    console_write("="*60)
    ...
    while True:
        ...
        if user_input.lower() == 'reload':
            console_write("✓ Context reloaded")
            continue
        ...
        if user_input.lower() == 'model':
            ...
            try:
                llm = LLMClient(provider, model, api_key)
                console_write("✓ Switched AI provider successfully")
                chat_history = []
            except Exception:
                logger.exception("Failed to switch model/provider")
                console_write("❌ Error: Could not switch model.")
            continue
        ...
        try:
            ...
            if provider == "openai":
                ...
                if message.tool_calls:
                    ...
                    console_write("\n" + "="*60)
                    console_write("🔧 AI wants to update the following data:")
                    console_write("="*60)
                    ...
                    for tool_call in message.tool_calls:
                        ...
                        console_write(f"\n📝 {readable_name}:")
                        console_write(json.dumps(args.get('data', args), indent=2))
            ...
            logger.exception("Unhandled error in interactive chat loop")
